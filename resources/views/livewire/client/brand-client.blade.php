<div class="w-full bg-white py-4 md:py-6 px-2 md:px-6">
    ...
    <div class=" flex items-center flex-wrap">
        @if ($products->isEmpty())
            <p class="whitespace-nowrap mx-auto text-xs md:text-sm">Không có Bài Đăng thuộc Thể Loại này.</p>
        @endif
                ...
                <a href="{{route('product-detail', ['id' => $product->id, 'slug' => $product->slug])}}">
                    <div class="w-full h-52 md:h-64">
                        @if (count($product->productDetails) > 0 && $product->productDetails[0] && $product->productDetails[0]->image)
                            ...
                            @endphp
                            <img src="{{ asset('storage/images/products/' . $imageThumbnail) }}" alt="Hình ảnh Bài Đăng" class="hover:grow hover:shadow-lg w-full h-full object-cover">
                        @else
                            <img src="{{ asset('library/images/image-not-found.jpg') }}" alt="Không có hình ảnh Bài Đăng" class="hover:grow hover:shadow-lg  w-full h-full object-cover">
                        @endif
                    </div>
